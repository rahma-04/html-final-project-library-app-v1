...
async function handleClickEditButton(bookId) {
  try {
    ...
    const getData = await fetch('http://localhost:3333/books/' + bookId);
    currentBook = await getData.json()
...
async function fetchBooks() {
  try {
    ...
    */
    const getData = await fetch('http://localhost:3333/books');
    books = await getData.json();
  } catch (error) {
    console.log(error);
    console.log('Terjadi kesalahan saat mengambil data buku');
  }
